!(function () {
  function addEventListener(element, event, cb) {
    element.addEventListener
      ? element.addEventListener(event, cb, !0)
  ...
  (videojs.Youtube = videojs.MediaTechController.extend({
    init: function (player, options, ready) {
      if (
        ...
        "undefined" != typeof options.source)
      )
        for (var key in options.source)
          options.source.hasOwnProperty(key) &&
      ...
      qualityContent.setAttribute("class", "vjs-control-content"),
        ...
        "undefined" !== player.options().quality &&
      ...
      this.parseSrc(player.options().src),
        ...
        (this.forceHTML5 = !(
          ...
          this.player_.options().forceHTML5 !== !0
        )),
      ...
      player.ready(function () {
        if (self.player_.options().controls) {
          var controlBar =
            self.playerEl_.querySelectorAll(".vjs-control-bar")[0];
          controlBar && controlBar.appendChild(self.qualityButton);
        }
        self.playOnReady &&
          !self.player_.options().ytcontrols &&
          ("undefined" != typeof self.player_.loadingSpinner &&
            self.player_.loadingSpinner.show(),
          "undefined" != typeof self.player_.bigPlayButton &&
    ...
    (videojs.Youtube.prototype.loadThumbnailUrl = function (id, callback) {
      var uri = "https://img.youtube.com/vi/" + id + "/maxresdefault.jpg",
        fallback = "https://img.youtube.com/vi/" + id + "/0.jpg";
      try {
        var image = new Image();
    ...
    (videojs.Youtube.prototype.updateIframeSrc = function () {
      var fullscreenControls =
          "undefined" == typeof this.player_.options().ytFullScreenControls ||
        ...
        isLocalProtocol =
          ...
          "app:" === window.location.protocol;
      isLocalProtocol && delete params.origin;
      for (var prop in params)
        !params.hasOwnProperty(prop) ||
          ("undefined" != typeof params[prop] && null !== params[prop]) ||
          delete params[prop];
      var self = this;
      if (this.videoId || this.playlistId) {
        if (
          ...
          (videojs.Youtube.loadingQueue.push(this), !videojs.Youtube.apiLoading)
        ) {
    ...
    (videojs.Youtube.prototype.addIframeBlocker = function (ieVersion) {
      (this.iframeblocker = videojs.Component.prototype.createEl("div")),
        ...
        ieVersion && 9 > ieVersion
          ? (this.iframeblocker.style.opacity = 0.01)
          : (this.iframeblocker.style.background = "rgba(255, 255, 255, 0.01)");
      var self = this;
      addEventListener(this.iframeblocker, "mousemove", function (e) {
        self.player_.userActive() || self.player_.userActive(!0),
          e.stopPropagation(),
          e.preventDefault();
      }),
    ...
    (videojs.Youtube.prototype.parseSrc = function (src) {
      if (((this.srcVal = src), src)) {
        var regId =
          ...
          match = src.match(regId);
        match && 11 === match[2].length
          ? (this.videoId = match[2])
          : (this.videoId = null);
        var regPlaylist = /[?&]list=([^#\&\?]+)/;
        (match = src.match(regPlaylist)),
          null !== match && match.length > 1
        ...
        (match = src.match(regVideoQuality)),
          null !== match &&
            ...
            videojs.Youtube.appendQualityLabel(
              this.qualityTitle,
              this.userQuality
            ));
      }
    ...
    (videojs.Youtube.prototype.src = function (src) {
      if ("undefined" != typeof src) {
        if ((this.parseSrc(src), "about:blank" === this.el_.src))
          return void this.updateIframeSrc();
        if ((delete this.defaultQuality, null !== this.videoId)) {
          this.player_.options().autoplay && this.playVideoIsAllowed
          ...
          this.loadThumbnailUrl(this.videoId, function (url) {
            (self.playerEl_.querySelectorAll(
              ".vjs-poster"
      ...
      null !== this.videoId &&
        ...
        this.isReady_
          ? (this.ytplayer.setVolume(100 * this.player_.volume()),
            this.volumeVal > 0 ? this.ytplayer.unMute() : this.ytplayer.mute(),
    ...
    (videojs.Youtube.prototype.setCurrentTime = function (seconds) {
      this.lastState === YT.PlayerState.PAUSED &&
        ...
        this.ytplayer.seekTo(seconds, !0),
    ...
    (videojs.Youtube.prototype.playbackRate = function () {
      return this.ytplayer && this.ytplayer.getPlaybackRate
        ? this.ytplayer.getPlaybackRate()
        : 1;
    }),
    ...
    (videojs.Youtube.prototype.setVolume = function (percentAsDecimal) {
      "undefined" != typeof percentAsDecimal &&
        percentAsDecimal !== this.volumeVal &&
        (this.ytplayer.setVolume(100 * percentAsDecimal),
    ...
    (videojs.Youtube.prototype.buffered = function () {
      if (this.ytplayer && this.ytplayer.getVideoBytesLoaded) {
        var loadedBytes = this.ytplayer.getVideoBytesLoaded(),
          totalBytes = this.ytplayer.getVideoBytesTotal();
        if (!loadedBytes || !totalBytes) return 0;
        var duration = this.ytplayer.getDuration(),
          secondsBuffered = (loadedBytes / totalBytes) * duration,
          secondsOffset =
    ...
    (videojs.Youtube.makeQueryString = function (args) {
      ...
      for (var key in args)
        args.hasOwnProperty(key) && array.push(key + "=" + args[key]);
      return array.join("&");
    }),
    ...
    (videojs.Youtube.prototype.updateCaptions = function () {
      this.ytplayer.loadModule("captions"), this.ytplayer.loadModule("cc");
      var options = this.ytplayer.getOptions(),
        cc =
          options.indexOf("captions") >= 0
            ? "captions"
            : options.indexOf("cc") >= 0
            ? "cc"
            : null;
      if (null !== cc && !this.tracked_) {
        var tracks = this.ytplayer.getOption(cc, "tracklist");
        if (tracks && tracks.length > 0) {
          for (var tt, i = 0; i < tracks.length; i++)
            tt = this.addTextTrack(
              "captions",
              tracks[i].displayName,
              tracks[i].languageCode
            );
          var self = this;
          this.textTracks().on("change", function () {
            for (var code = null, i = 0; i < this.length; i++)
              if ("showing" === this[i].mode) {
    ...
    (videojs.Youtube.prototype.updateQualities = function () {
      ...
      var qualities = this.ytplayer.getAvailableQualityLevels(),
        self = this;
      if (
        (qualities.indexOf(this.userQuality) < 0 &&
          videojs.Youtube.appendQualityLabel(
            self.qualityTitle,
            this.defaultQuality
          ),
      ...
      if (state !== this.lastState) {
        switch (state) {
          ...
          case YT.PlayerState.ENDED:
            var stopPlaying = !0;
            this.playlistId &&
              !this.player_.options().loop &&
              (stopPlaying = 0 === this.ytplayer.getPlaylistIndex()),
              stopPlaying &&
                (this.player_.options().ytcontrols ||
                  ((this.playerEl_.querySelectorAll(
                    ".vjs-poster"
    ...
    (videojs.Youtube.appendQualityLabel = function (element, quality) {
      ...
      switch ((label.setAttribute("class", "vjs-hd-label"), quality)) {
        case "hd720":
        case "hd1080":
        case "hd1440":
          setInnerText(label, "HD"), element.appendChild(label);
          break;
        case "hd2160":
          setInnerText(label, "4K"), element.appendChild(label);
      }
    }),
    (videojs.Youtube.prototype.onPlaybackQualityChange = function (quality) {
      if (
        ...
        "undefined" == typeof this.userQuality)
      ) {
        switch (
    ...
    (videojs.Youtube.addClass = function (element, classToAdd) {
      -1 === (" " + element.className + " ").indexOf(" " + classToAdd + " ") &&
        (element.className =
          "" === element.className
            ? classToAdd
    ...
    (videojs.Youtube.removeClass = function (element, classToRemove) {
      ...
      if (-1 !== element.className.indexOf(classToRemove)) {
        for (
          classNames = element.className.split(" "), i = classNames.length - 1;
          i >= 0;
          i--
        )
